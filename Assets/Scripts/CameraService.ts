...
export class CameraService extends BaseScriptComponent {
  ...
  start() {
    ...
    // camTexControl.onNewFrame.add(() => {}); // TODO
  ...
  private registeredEvent: EventRegistration = null;
  toggleStream(b: boolean) {
    const camTexControl = this.camTexture.control as CameraTextureProvider;
    if (b) {
      this.registeredEvent = camTexControl.onNewFrame.add(() => {});
    } else {
      camTexControl.onNewFrame.remove(this.registeredEvent);
    }
  }
